...
class FlynnApp {
  ...
  async withConfig (config = {}, deployFn) {
    ...
    let routesToAdd
    if (config.routes) {
      routesToAdd = await this._ensureRoutes(config.routes)
    }

    await deployFn()

    if (routesToAdd) {
      await this._ensureScale(routesToAdd)
    }
  }
  ...
  async _ensureResources (resources) {
    ...
    const newResources = await Promise.all(
      resources.filter(resource => {
        return !existingResources.find(r => r.provider === resource)
      }).map(resourceToAdd => {
        debug('Adding resource %s', resourceToAdd)
        return this.apiClient.post(`/providers/${resourceToAdd}/resources`, {
          apps: [this.id]
        })
      })
    )
  ...
  async _ensureRoutes (routes) {
    const routesToAdd = clone(routes)

...
class FlynnApp {
  ...
  async withConfig (config = {}, deployFn) {
    ...
    await this._ensureEnv(Object.assign({}, resourcesEnv, config.env))
...
module.exports = class FlynnService {
  ...
  async createApp (appName) {
    ...
    const {id: newReleaseId} = await this.apiClient.post('/releases', initRelease)
    await this.apiClient.post(`/apps/${id}/deploy`, {id: newReleaseId})
